...
/// <summary>
/// Copy of <see cref="AssemblyRunner" /> which makes it possible to discover and run tests independently
/// </summary>
public class CustomAssemblyRunner : LongLivedMarshalByRefObject, IDisposable, IMessageSinkWithTypes
{
    ...
    public IReadOnlyCollection<ITestCase> TestCasesToRun => testCasesToRun.AsReadOnly();
    ...
    private readonly AppDomainSupport _appDomainSupport;
    private readonly string _assemblyFileName;
    private readonly string _configFileName;
    private readonly bool _shadowCopy;
    private readonly string _shadowCopyFolder;

    CustomAssemblyRunner(AppDomainSupport appDomainSupport,
                   ...
                   string shadowCopyFolder = null)
    {
        ...
        _appDomainSupport = appDomainSupport;
        _shadowCopy = shadowCopy;
        _shadowCopyFolder = shadowCopyFolder;
                      ...
                      bool? internalDiagnosticMessages = null)
    {
        ...
        controller = new XunitFrontController(_appDomainSupport, _assemblyFileName, _configFileName, _shadowCopy, _shadowCopyFolder, diagnosticMessageSink: MessageSinkAdapter.Wrap(this));
        ...
        XunitWorkerThread.QueueUserWorkItem(() =>
        {
            ...
            var executionOptions = GetExecutionOptions(diagnosticMessages, parallel, maxParallelThreads, internalDiagnosticMessages);
            controller.RunTests(testCases, this, executionOptions);
            executionCompleteEvent.WaitOne();
            executionCompleteEvent.Reset();
        });
    }

    public void Discover(string typeName = null,
                      bool? diagnosticMessages = null,
                      TestMethodDisplay? methodDisplay = null,
                      TestMethodDisplayOptions? methodDisplayOptions = null,
                      bool? preEnumerateTheories = null,
    ...
    /// </summary>
    /// <param name="assemblyFileName">The test assembly.</param>
    public static CustomAssemblyRunner WithoutAppDomain(string assemblyFileName)
    {
        return new CustomAssemblyRunner(AppDomainSupport.Denied, assemblyFileName);
    }
